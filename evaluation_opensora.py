...
from taming.opensora_2d.vae import VideoVQVAEPipeline
...
def load_vqgan_new(config, ckpt_path=None, is_gumbel=False):
  model = VideoVQVAEPipeline(**config.model.init_args)
...
def main(args):
    ...
    with torch.no_grad():
        for batch in tqdm(dataset._val_dataloader()):
            ...
            quant, _, _, _, indices, _ = model.encode(images)
            # reconstructed_images, _ = model.decode(quant)
            ### usage
    ...
    f = open(f"{args.ckpt_path.split('/')[-3]}.json", "w")
    f.write(usage)
    f.close()
