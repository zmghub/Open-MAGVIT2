...
from .utils import TemporalDiagonalGaussianDistribution
...
class CausalConv2d(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        #self.time_pad = time_pad
        self.time_causal_padding = (width_pad, width_pad, height_pad, height_pad)

        stride = strides if strides is not None else (1, 1)
        if len(stride) > 2:
...
class ResBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = conv_fn(in_channels, self.filters, kernel_size=3, bias=False)
        self.norm2 = nn.GroupNorm(num_groups, self.filters)
        self.conv2 = conv_fn(self.filters, self.filters, kernel_size=3, bias=False)
        if in_channels != filters:
            if self.use_conv_shortcut:
                self.conv3 = conv_fn(in_channels, self.filters, kernel_size=3, bias=False)
            else:
                self.conv3 = conv_fn(in_channels, self.filters, kernel_size=1, bias=False)
...
class Encoder3D(nn.Module):
    ...
    def __init__(
        ...
        # 如果第一帧要单独编码，则添加一个单独的卷积层
        self.separate_first_frame_encoding = separate_first_frame_encoding
        if separate_first_frame_encoding:
            self.conv_in_first_frame = CausalConv2d(in_out_channels, filters, kernel_size=(3, 3))
            ...
            if i < self.num_blocks - 1:
                if self.temporal_downsample[i]:
                    ...
                    self.conv_blocks.append(
                        self.conv_fn(
                            prev_filters, filters, kernel_size=3, strides=(t_stride, s_stride, s_stride)
                        )
                    )
                    prev_filters = filters  # update in_channels
                else:
        ...
        self.conv2 = self.conv_fn(prev_filters, self.embedding_dim, kernel_size=1, padding="same")

    def forward(self, x, time_padding=0):
        x_len = x.shape[2]
        if x_len == 1:
            x = self.conv_in_first_frame(x)
            x = pad_at_dim(x, (time_padding, 0), dim=2)   
        elif self.separate_first_frame_encoding:
            packed_shape = [torch.Size([]), torch.Size([x_len - 1])]
            first_frame, x = unpack(x, packed_shape, 'b c * h w')          # 拆出首帧
            first_frame = self.conv_in_first_frame(first_frame)            # 首帧编码
            x = self.conv_in(x)
            x, _ = pack([first_frame, x], 'b c * h w')                     # 单独编码后合并
            x = pad_at_dim(x, (time_padding, 0), dim=2)                    # padding
        else:
            x = pad_at_dim(x, (time_padding, 0), dim=2)                    # 时序padding
            x = self.conv_in(x)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(self.num_blocks):
            ...
            if i < self.num_blocks - 1:
                if self.temporal_downsample[i]:
                    ...
                    self.conv_blocks.append(
                        self.conv_fn(
                            prev_filters, filters, kernel_size=(3, 3), strides=(s_stride, s_stride)
                        )
                    )
                    prev_filters = filters  # update in_channels
                else:
        ...
        self.conv2 = self.conv_fn(prev_filters, self.embedding_dim, kernel_size=(1, 1), padding="same")
...
class Decoder3D(nn.Module):
    ...
    def __init__(
        ...
        # last conv
        self.conv1 = self.conv_fn(self.embedding_dim, filters, kernel_size=3, bias=True)
            ...
            if i > 0:
                if self.temporal_downsample[i - 1]:
                    ...
                    self.conv_blocks.insert(
                        0,
                        self.conv_fn(
                            prev_filters, prev_filters * t_stride * self.s_stride * self.s_stride, kernel_size=3
                        ),
                    )
                else:
                    self.conv_blocks.insert(
        ...
        self.separate_first_frame_encoding = separate_first_frame_encoding
        if separate_first_frame_encoding:
            self.conv_out_first_frame = CausalConv2d(filters, in_out_channels, kernel_size=(3, 3))
    ...
    def forward(self, x, time_padding=0):
        ...
        x_len = x.shape[2]
        if x_len - time_padding == 1:
            x = x[:, :, time_padding:]
            x = self.conv_out_first_frame(x)
        elif self.separate_first_frame_encoding:
            packed_shape = [torch.Size([time_padding]), torch.Size([]), torch.Size([x_len - 1 - time_padding])]
            pad, first_frame, x = unpack(x, packed_shape, 'b c * h w')          # 拆出首帧
            first_frame = self.conv_out_first_frame(first_frame)                # 首帧解码
            x = self.conv_out(x)
            x, _ = pack([first_frame, x], 'b c * h w')                          # 单独解码后合并
        else:
            x = self.conv_out(x)
            x = x[:, :, time_padding:]                                          # 去除padding区域
        return x
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        # last conv
        self.conv1 = self.conv_fn(self.embedding_dim, filters, kernel_size=(3, 3), bias=True)
    ...
    def forward(self, x, time_padding=0):
        ...
        for i in reversed(range(self.num_blocks)):
            ...
            if i > 0:
                ...
                x = rearrange(
                    x,
                    "B (C ts hs ws)  H W -> B C ts (H hs) (W ws)",
                    ts=t_stride,
                    hs=self.s_stride,
                    ws=self.s_stride,
                )
...
class VAE_Temporal(nn.Module):
    ...
    def encode(self, x):
        ...
        encoded_feature = self.encoder(x, time_paddings=time_padding)
        moments = self.quant_conv(encoded_feature).to(x.dtype)
        posterior = TemporalDiagonalGaussianDistribution(moments)
        return posterior
    ...
    def decode(self, z, num_frames=None):
        ...
        x = self.decoder(z, time_paddings=time_padding)
        return x
...
class VQVAE_Temporal(nn.Module):
    def __init__(
        self,
        in_out_channels=4,
        latent_embed_dim=18,
        ...
        self.quant_conv = CausalConv3d(latent_embed_dim, embed_dim, 1)
    ...
    def encode(self, x):
        ...
        encoded_feature = self.encoder(x, time_paddings=time_padding)
        z = self.quant_conv(encoded_feature).to(x.dtype)
        return z
    ...
    def decode(self, z, num_frames=None):
        ...
        z = self.post_quant_conv(z)
        x = self.decoder(z, time_padding=time_padding)
        return x

    def forward(self, x):
        z = self.encode(x)
        recon_video = self.decode(z, num_frames=x.shape[2])
        return recon_video, z


class VQVAE_Image(nn.Module):
    def __init__(
        self,
        in_out_channels=4,
        latent_embed_dim=18,
        ...
        self.quant_conv = CausalConv2d(latent_embed_dim, embed_dim, 1)

        self.post_quant_conv = CausalConv2d(embed_dim, latent_embed_dim, 1)
