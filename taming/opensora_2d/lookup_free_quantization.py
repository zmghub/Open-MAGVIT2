...
class LFQ(Module):
    def __init__(
        ...
        *,
        ...
        assert exists(codebook_size), 'codebook_size must be specified for LFQ'
        assert log2(codebook_size).is_integer(), f'your codebook size must be a power of 2 for lookup free quantization (suggested {2 ** ceil(log2(codebook_size))})'
...
if __name__ == "__main__":
    x = torch.randn(1, 18, 8*8*4).cuda()
    lfq = LFQ(codebook_size=262144, num_codebooks=1).to("cuda")
    # lfq.eval()
    ret = lfq(x)